...
public class QuoteRepository {
  ...
  public Completable save(String token, Quote quote) {
    if (quote.getId() == null) {
      return  Completable.fromSingle(proxy.post(String.format(OAUTH_HEADER_FORMAT, token), quote)
          .subscribeOn(Schedulers.from(networkPool))
      );
    } else {
      return  Completable.fromSingle(proxy.put(String.format(OAUTH_HEADER_FORMAT, token), quote, quote.getId())
          .subscribeOn(Schedulers.from(networkPool))
      );
    }
  }
